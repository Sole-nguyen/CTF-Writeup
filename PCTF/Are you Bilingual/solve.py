...
# Convert to proper byte range:
out = [(x + 256) % 256 for x in output]
...
# decode functions
def dec_first(v):
    return chr(~(v ^ first_val) & 0xFF)

def dec_second(v):
    return chr(~(v ^ second_val) & 0xFF)

first_half  = ''.join(dec_first(v) for v in first)
second_half = ''.join(dec_second(v) for v in second)

art_str = first_half + second_half

print("Recovered art_str first 20 chars:", art_str[:20])

# ---- Extract flag directly ----
flag_chars = []
start = len(art_str) % 10

pos = start
while pos < len(art_str):
