...
print("Attempting ECM factorization...")
print(f"n = {n}")
print(f"bits: {n.nbits()}")

# Try ECM with reasonable bounds for CTF
try:
    print("\nTrying ECM with B1=10000...")
    result = ecm.factor(n, B1=10000, verbose=True)
    print(f"Result: {result}")
except:
    print("ECM with B1=10000 failed or timed out")

try:
    print("\nTrying ECM with B1=50000...")
    result = ecm.factor(n, B1=50000, verbose=True)
    print(f"Result: {result}")
except:
    print("ECM with B1=50000 failed or timed out")
